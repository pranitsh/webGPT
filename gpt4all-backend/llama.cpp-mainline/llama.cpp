...
static struct ggml_cgraph * llm_build_mpt(
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // TODO: refactor into common function (shared with LLaMA)
        {
            ...
            struct ggml_tensor * KQ_scaled_alibi =
                ggml_alibi(ctx0, KQ_scaled, n_past, n_head, max_alibi_bias);
            offload_func_kq(KQ_scaled_alibi);
            ggml_set_name(KQ_scaled_alibi, "KQ_scaled_alibi");

            struct ggml_tensor * KQ_masked = ggml_diag_mask_inf_inplace(ctx0, KQ_scaled_alibi, n_past);
            offload_func_kq(KQ_masked);
            ggml_set_name(KQ_masked, "KQ_masked");
...
static struct ggml_cgraph * llama_build_graph(
    ...
    switch (model.arch) {
        ...
        case LLM_ARCH_MPT:
            {
                result = llm_build_mpt(lctx, tokens, embd, n_tokens, n_past);
            } break;
        default:
            GGML_ASSERT(false);
    };
